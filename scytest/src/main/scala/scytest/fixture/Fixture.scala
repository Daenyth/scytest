...
sealed abstract class Fixture[F[_], R] {
  ...
  type DTags <: Product
  val tagMapper: tuple.Mapper[DTags, toTag.type]
  val dependencies: tagMapper.Out

  val tagFn: TagFn[DTags]

  /** Construct the resource from the initialized dependencies */
  def resource(dependency: tagFn.Objs): Resource[F, R]
...
sealed trait RootFixture[F[_], R] extends Fixture[F, R] {
  final type DTags = Unit
  override val tagMapper: tuple.Mapper[Unit, toTag.type] = implicitly
  override val dependencies: tagMapper.Out = ()

  override val tagFn: TagFn[Unit] = implicitly
  val resource_ : Resource[F, R]
  override final def resource(d: tagFn.Objs): Resource[F, R] = resource_
}
...
private[fixture] class MappedFixture[F[_]: Applicative, R, R2, D0 <: TList](
  ...
  override val tag: FixtureTag.Aux[R2] =
    FixtureTag[R2](s"Mapped(${fixture.tag}, $f)", fixture.tag.scope)
  type DTags = D0
  override val dependencies: DTags = fixture.dependencies
...
private[fixture] class RenamedFixture[F[_], R, D0 <: TList](
  ...
  override val tag: FixtureTag.Aux[R] = FixtureTag[R](name, base.tag.scope)
  type DTags = D0
  override val dependencies: DTags = base.dependencies
...
private[fixture] class ResourceFixture[F[_], R](
    override val resource_ : Resource[F, R],
    override val tag: FixtureTag.Aux[R]
...
private[fixture] class MapNFixture[F[_], D1, D2, R](
  ...
  override val dependencies: DTags = fix1.tag ::: fix2.tag ::: TNil

  def resource(d: D): Resource[F, R] = f(d.head, d.tail.head)

  override val tag: FixtureTag.Aux[R] =
    FixtureTag[R](
      s"ProductFixture(${fix1.tag.name}, ${fix2.tag.name})",
...
private[fixture] class FlatMapFixture[F[_]: Monad, D0, RGet](
  ...
  override val dependencies: DTags = dependency ::: TNil

  def resource(dependency: D) = f(dependency.head)

  override val tag: FixtureTag.Aux[RGet] =
...
private[fixture] class PureFixture[F[_], R](value: R, tagName: String)(
  ...
  override val tag: FixtureTag.Aux[R] =
    FixtureTag[R](tagName, FixtureScope.Process)
  override val resource_ = Resource.pure[F, R](value)
}
...
private[scytest] class ContextShiftFixture[F[_]](
  ...
  override val tag: FixtureTag.Aux[ContextShift[F]] = ContextShiftFixture.tag[F]
...
private[scytest] class TimerFixture[F[_]](
  ...
  override val tag: FixtureTag.Aux[Timer[F]] = TimerFixture.tag[F]
